...
def cartesian_to_polar(points):
    """Преобразование из декартовых координат [x, y] в полярные [r, theta].
    Вход: [N, 2, C], где 2 — это [x, y].
    Выход: [N, 2, C], где 2 — это [r, theta].
    """
...
def polar_to_cartesian(points):
    """Преобразование из полярных координат [r, theta] в декартовы [x, y].
    Вход: [N, 2, S], где 2 — это [r, theta].
    Выход: [N, 2, S], где 2 — это [x, y].
    """
    r = points[:, 0, :]  # длины векторов, [512, 100]
    theta = points[:, 1, :]  # углы, [512, 100]
    x = r * torch.cos(theta)  # [512, 100]
    y = r * torch.sin(theta)  # [512, 100]
    return torch.stack([x, y], dim=1)  # [512, 2, 100]


def polar_to_cartesian_test(points):
    r = points[..., 0]  # [102, 16, 100] — длины векторов
    theta = points[..., 1]  # [102, 16, 100] — углы
    x = r * torch.cos(theta)  # [102, 16, 100]
    y = r * torch.sin(theta)  # [102, 16, 100]
    return torch.stack([x, y], dim=-1)  # [102, 16, 100, 2]    



@TRAINERS_REGISTRY.add_to_registry(name="base_pol")
class BaseTrainerWiAllMetrics:
    ...
    def forward_model(self, data_dict):
        ...
        t, result["batch_weights"] = self.schedule_sampler.sample(dd['house'].shape[0], self.config.training.device)
        result["target"] = torch.randn_like(dd['house'])
        d = dd['house']
        d = cartesian_to_polar(dd['house'])
        # print(d.shape)

        r = d[:, 0, :]  
        theta = d[:, 1, :]  
        r_norm = r / math.sqrt(2)
        theta_norm = theta / (2 * math.pi)
        # -1 1 
        r_norm = r_norm * 2 - 1
        theta_norm = theta_norm * 2 - 1

        d = torch.stack([r_norm, theta_norm], dim=1)



        x_t = self.diffusion.q_sample(d, t, noise=result["target"])
        ...
        if not self.config.data.analog_bit:
            ...
            result["bin_target"] = dd['house'].detach()
            ...
            result["bin_target"] = result["bin_target"].reshape([result["target"].shape[0], result["target"].shape[2], 2 * self.diffusion.num_bits]).permute([0,2,1])
    ...
    def forward_model_test(self, data_dict):
        ...
        result = self.sample_with_gt(dd["house"], dd)
        return result
    ...
    def train_epoch(self, epoch_num, epoch_info):
        ...
        # add agregated means values to epoch_info

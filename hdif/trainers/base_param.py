...
@TRAINERS_REGISTRY.add_to_registry(name="base_param")
class BaseTrainerWiAllMetrics:
    ...
    def setup_networks(self):
        ...
        if self.config.checkpoint.checkpoint4load is not None:
            ...
            #     print(f'!!!!! Cant load the model. Exception: {e}')
    ...
    def forward_model(self, data_dict):
        dd = data_dict 
        result = dict()
        tmp = dd["room_offsets_centers"]
        b_s = tmp.shape[0]

        t, result["batch_weights"] = self.schedule_sampler.sample(b_s, self.config.training.device) 
        
        result["target"] = torch.randn_like(tmp)

        x_t = self.diffusion.q_sample(tmp, t, noise=result["target"]) #512 2 100 -> 512 4 100
        
        result["tmp_mask"] = (1 - dd['src_key_padding_mask']) 
        ...
        result["model_output_dec"], result["model_output_bin"] = self.model(x_t, self.diffusion._scale_timesteps(t), xtalpha=xtalpha, epsalpha=epsalpha, **dd)
        ...
        if not self.config.data.analog_bit:
            ...
            result["bin_target"] = result["bin_target"].reshape([result["target"].shape[0], result["target"].shape[2],  4 * self.diffusion.num_bits]).permute([0,2,1])
            
            result["t_weights"] = (t<10).to(device).unsqueeze(1).unsqueeze(2)
    ...
    def sample_with_gt(self, data_sample_gt, model_kwargs):
        print("SAMPLE_WITH_GT")
        sample_dict = self.sample(data_sample_gt.shape, model_kwargs)
        sample = sample_dict["samples"]
        sample_gt = model_kwargs['house'].unsqueeze(0)
        
        only_sample_i = sample[-1:].squeeze(0)  #(bs, 4, 100)
        sample_i_original = only_sample_i.clone().detach()
        
        room_indices = model_kwargs['room_indices']  # (bs, 100, 32)
        bs = only_sample_i.shape[0]        

        # усреднение центров
        for b in range(bs):
            center_x = only_sample_i[b, 2, :]
            center_y = only_sample_i[b, 3, :]

            rooms_b = room_indices[b] 
            room_id_per_point = torch.argmax(rooms_b, dim=1)

            sum_x_per_room = torch.sum(center_x.unsqueeze(1) * rooms_b, dim=0)
            sum_y_per_room = torch.sum(center_y.unsqueeze(1) * rooms_b, dim=0)

            count_per_room = torch.sum(rooms_b, dim=0) #  количество точек в каждой комнсте

            avg_center_x_per_room = torch.zeros_like(sum_x_per_room)
            avg_center_y_per_room = torch.zeros_like(sum_y_per_room)

            mask_nonempty = (count_per_room > 0)
            avg_center_x_per_room[mask_nonempty] = sum_x_per_room[mask_nonempty] / count_per_room[mask_nonempty]
            avg_center_y_per_room[mask_nonempty] = sum_y_per_room[mask_nonempty] / count_per_room[mask_nonempty]

            center_x = avg_center_x_per_room[room_id_per_point]
            center_y = avg_center_y_per_room[room_id_per_point]

            only_sample_i[b, 2, :] = center_x
            only_sample_i[b, 3, :] = center_y

        sample_i = only_sample_i.unsqueeze(0)  # (1, bs, 4, 100)
        sample[-1] = sample_i[0]
        ...
        print(f'ID {model_kwargs["id"]} \n \n')
        return {"sample_and_gt": sample_and_gt}
        
    def forward_model_test(self, data_dict):
        dd = data_dict
        result = dict()
        # Логируем содержимое room_offsets_centers
        house = dd["room_offsets_centers"].cpu()
        result = self.sample_with_gt(house, dd)
        return result
    ...
    def update_lr(self):
        if self.train_iter % self.config.training.upd_lr_iter == 0:
            lr = self.optim_model.param_groups[0]['lr'] * (0.1**(self.train_iter//self.config.training.upd_lr_iter)) #ТУТ
    ...
    def test_epoch(self, epoch_num, epoch_info):
        ...
        for iter_num in tqdm(range(self.num_test_iters)):
            ...
            outputs = self.forward_model_test(batch)

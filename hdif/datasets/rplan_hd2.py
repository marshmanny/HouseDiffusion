...
def load_rplanhg_data(
    ...
    dataset = HDRPlanDataset(set_name, analog_bit, target_set, base_dir=base_dir)
    if deterministic:
        loader = DataLoader(
...
@DATASETS_REGISTRY.add_to_registry("rplan-hd", ("train", "val", "test"))
class HDRPlanDataset(Dataset):
    def __init__(self, set_name, analog_bit, target_set, non_manhattan=False, base_dir='../datasets/rplan', save_path="save_path", max_len=None):
        super().__init__()
        ...
        if os.path.exists(f'{save_path}/rplan_{set_name}_{target_set}.npz'):
            ...
            try:
                np.savez_compressed(f'{save_path}/rplan_{set_name}_{target_set}',
                                    graphs=self.graphs,
                                    houses=self.houses,
            ...
            if set_name=='eval':
                ...
                try:
                    np.savez_compressed(f'{save_path}/rplan_{set_name}_{target_set}_syn',
    ...
    def __getitem__(self, idx):
        ...
        if not self.analog_bit:
            ...
            return_arr = arr_onehot.astype(float)
        
        cond["house"] = return_arr
        return cond
...
@DATASETS_REGISTRY.add_to_registry("rplan-hd-test-full")
class HDRPlanTestFullDataset(HDRPlanDataset):
    def __init__(self):
        super().__init__('eval', False, 8, non_manhattan=False, base_dir='data/rplan_hdiff', save_path="data/rplan_processed_hdiff", max_len=1000)

@DATASETS_REGISTRY.add_to_registry("rplan-hd-test-small")
class HDRPlanTestSmallDataset(HDRPlanDataset):
    def __init__(self):
        super().__init__('eval', False, 8, non_manhattan=False, base_dir='data/rplan_hdiff', save_path="data/rplan_processed_hdiff", max_len=30)


if __name__ == '__main__':
    dataset = HDRPlanDataset('eval', False, 8)

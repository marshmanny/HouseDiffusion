...
def convert_to_center_offsets(corners):
    ...
    center = np.mean(corners, axis=0)  # Центр комнаты (cx, cy)
    offsets = corners - center  # Смещения углов относительно центра
...
def convert_to_corners(center, offsets):
    ...
    """
    return center + offsets  # Просто прибавляем смещения к центру
...
class MSDRPlanStructuralDataset(Dataset):
    def __init__(self, set_name, analog_bit, target_set, non_manhattan=False, use_structural_elements=True, save_path="data/processed_rplan_structural_full", data_zip_path="data/rplan_json.zip", max_len=None):
        ...
        # if os.path.exists(f'{save_path}/rplan_{set_name}_{target_set}.npz'):
            ...
            self.houses_with_center_offsets = []
            ...
            for h in self.houses:
                if h.shape[0] > 0:
                    dense_house = np.asarray(h.todense())  # Преобразование sparse в numpy
                    center, offsets = convert_to_center_offsets(dense_house[:, :2])
                    self.houses_centers.append(center)
                    self.houses_offsets.append(offsets)
                else:
                    self.houses_centers.append(None)
                    self.houses_offsets.append(None)
            ...
            for file_name in tqdm(file_names[:100]): #тут с помощью reader читаем входные данные 
        ...
        with open("exptxt/INTTRAIN.txt", "w") as log_file:
            log_file.write(f"x shape: {len(self.houses_centers)}\n")
            log_file.write(f"x shape: {len(self.houses_offsets)}\n")
    ...
    def __getitem__(self, idx):
        ...
        if self.houses_centers[idx] is not None:
            cond['center'] = self.houses_centers[idx]
            cond['offsets'] = self.houses_offsets[idx]
        else:
            cond['center'] = None
            cond['offsets'] = None
        
        
        with open("exptxt/getitem.txt", "w") as log_file:
            log_file.write(f"x shape: {len(cond['center'])}\n")
            log_file.write(f"x shape: {len(cond['offsets'])}\n")

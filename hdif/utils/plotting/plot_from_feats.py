...
def plot_predictions(sample_and_gt, dpi=100):
    ...
    for i in range(batch_size):
        fig, axs = plt.subplots(2, 1, figsize=(20, 10), dpi=dpi)
        
        pred_img = plot_from_batch(sample, model_kwargs, i, ax=axs[0])
        gt_img = plot_from_batch(sample_gt, model_kwargs, i, ax=axs[1])
        
        # img_sub2 = np.zeros_like(pred_img)[:, :]
        # img_ref2 = np.zeros_like(gt_img)[:, :]
        
        # for cl in CMAP_ROOMTYPE:
        #     mask = (img_sub == cl).all(-1)
        #     if mask.size != 0:
        #         img_sub2[mask] = cl
            
        #     mask = (img_ref == cl).all(-1)
        #     if mask.size != 0:
        #         img_ref2[mask] = cl
        ...
        pred_gt_imgs.append({"pred": pred_img, "gt": gt_img})

        axs[0].set_title(f"prediction {model_kwargs['id'][i]}")
        axs[1].set_title(f"ground truth {model_kwargs['id'][i]}")

...
from hdif.models.transformer import TransformerModel
...
@MODELS_REGISTRY.add_to_registry("GaussianDiffusion-param")
class GaussianDiffusion:
    """
    Utilities for training and sampling diffusion models.
    ...
    def p_mean_variance(
        ...
        if t[0] < predict_descrete:
            ...
            model_output_bin[model_output_bin<=0] = 0
            model_output_bin = bin2dec(model_output_bin.round().int().permute([0,2,1]).reshape(model_output_bin.shape[0],
                model_output_bin.shape[2], 4, self.num_bits), self.num_bits).permute([0,2,1]) #ТУТ
...
@MODELS_REGISTRY.add_to_registry("GaussianDiffusion-pol")
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        def process_xstart(x):
            ...
            #     # r_max = 1.2
            #     # r_min = 0
            #     r = th.clamp(r, 0.00, 1.414)
            #     theta = th.clamp(theta, 0.0, 2 * math.pi)
            #     x = th.stack([r, theta], dim=-1)

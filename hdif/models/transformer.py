...
def polar_to_cartesian(points):
    r = points[..., 0]  # [512, 100]
    theta = points[..., 1]  # [512, 100]
    x = r * th.cos(theta)  # [512, 100]
    y = r * th.sin(theta)  # [512, 100]
    return th.stack([x, y], dim=-1)  # [512, 100, 2]


def average_points_polar(point1: th.Tensor, point2: th.Tensor) -> th.Tensor:

    r1, theta1 = point1[..., 0, :], point1[..., 1, :]
    r2, theta2 = point2[..., 0, :], point2[..., 1, :]

    r_new = (r1 + r2) / 2

    cos_mean = (th.cos(theta1) + th.cos(theta2)) / 2
    sin_mean = (th.sin(theta1) + th.sin(theta2)) / 2
    theta_new = th.atan2(sin_mean, cos_mean)

    return th.stack([r_new, theta_new], dim=-2)  #
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        print(f'analog bit {self.analog_bit}')
        ...
        print(f"Created model with {self.use_structural_cross_attention=} and {self.struct_in_channels=}") #false
    ...
    def forward(self, x, timesteps, xtalpha, epsalpha, is_syn=False, x_structural: th.Tensor=None, **kwargs) -> typing.Tuple[th.Tensor, th.Tensor]:
        ...
        if not self.analog_bit:
            ...
            if self.use_structural_cross_attention:
                ...
                # print(f' 12345: {x_structural.shape}')
                ...
                # print(f'BASE: {x_structural.shape},{ struct_bin_input.shape}, {self.in_channels}')
...
@MODELS_REGISTRY.add_to_registry("Transform_param")
class TransformerModel(nn.Module):
    """
    The full Transformer model with timestep embedding.
    """
    ...
    def __init__(
        ...
        if self.use_structural_cross_attention:
            ...
            if self.use_wall_self_attention:
                self.wall_self_attention_layers = nn.ModuleList([WallEncoderLayer(self.model_channels, 4, 0.1, self.activation) for x in range(self.num_transfomers_layers)]) # type: ignore
                print(self.wall_self_attention_layers)
        ...
        if not self.analog_bit:
            self.output_linear_bin1 = nn.Linear(self.in_channels + self.in_channels * self.num_bits + self.model_channels, self.model_channels)
            ...
            self.output_linear_bin4 = nn.Linear(self.model_channels, 4 * self.num_bits)
    ...
    def expand_points(self, points: th.Tensor, connections: th.Tensor) -> th.Tensor:
        """
        Для входного тензора points размерности (B, N, 4), где:
        - channels 0:2 содержат смещения (offsets),
        - channels 2:4 содержат центр (center),
        функция расширяет только смещения, добавляя промежуточные точки вдоль стены (на основе connections),
        а затем конкатенирует неизменённые центры.
        
        Итоговый тензор будет иметь размерность (B, N, 20):
        первые 18 каналов – расширенные смещения,
        последние 2 – центр.
        
        Кроме того, все промежуточные размерности записываются в файл "txtt/expand_points_shapes.txt".
        """
        offsets = points[..., :2]   # (B, N, 2)
        centers = points[..., 2:]   # (B, N, 2)

        def average_points(point1, point2):
            return (point1 + point2) / 2.0

        p1 = offsets.view(offsets.shape[0], offsets.shape[1], 2, -1)  # (B, N, 2, 1)
 
        p5 = offsets[th.arange(offsets.shape[0])[:, None], connections[:, :, 1].long()]
        p5 = p5.view(p5.shape[0], p5.shape[1], 2, -1)  # (B, N, 2, 1)
        ...
        p1_ = p1.view_as(offsets)
        p1_5_ = p1_5.view_as(offsets)
        p2_ = p2.view_as(offsets)
        p2_5_ = p2_5.view_as(offsets)
        p3_ = p3.view_as(offsets)
        p3_5_ = p3_5.view_as(offsets)
        p4_ = p4.view_as(offsets)
        p4_5_ = p4_5.view_as(offsets)
        p5_ = p5.view_as(offsets)

        expanded_offsets = th.cat((p1_, p1_5_, p2_, p2_5_, p3_, p3_5_, p4_, p4_5_, p5_), dim=2)
       
        repeated_centers = centers.repeat(1, 1, 9)
        new_points = th.cat((expanded_offsets, repeated_centers), dim=2)
    ...
    def forward(self, x, timesteps, xtalpha, epsalpha, is_syn=False, x_structural: th.Tensor=None, **kwargs) -> typing.Tuple[th.Tensor, th.Tensor]:
        ...
        # x is  i think batch_size x num_coordinates x number of corners
        x = x.permute([0, 2, 1]).float() # -> convert [N x C x S] to [N x S x C] # 512 100 2 -> 512 100 4


        # So now, x is batch_size x number of corners x num_coordinates (.i.e. N x corners x features)

        if not self.analog_bit: #сюда мы заходим
            x = self.expand_points(x, kwargs[f'{prefix}connections'])
        ...
        #x.shape при house - это [512, 100, 18]
        input_emb = self.input_emb(x) 
        ...
        # with open("txtt/emb.txt", "w") as f:
        #     f.write(f" input_emb {input_emb.shape}\n") 
        #     f.write(f" cond_emb {cond_emb.shape}\n") 
        #     f.write(f" time_emb {time_emb.shape}\n")  
        #     f.write(f" time_emb repeat{time_emb.repeat((1, input_emb.shape[1], 1)).shape}\n") 
        ...
        out_dec = self.activation(out_dec)
        out_dec = self.output_linear2(out_dec)
        out_dec = self.output_linear3(out_dec) # 512 100 2 -> 512 100 4
        

        if not self.analog_bit:

            xtalpha_offset = xtalpha[:, :, 0:2]  # (B=512, N=100, 2)
            xtalpha_center = xtalpha[:, :, 2:4] 
            xtalpha_offset_18 = xtalpha_offset.repeat(1, 1, 9)
            xtalpha_center = xtalpha_center.repeat(1,1,9)

            xtalpha_36 = th.cat([xtalpha_offset_18, xtalpha_center], dim=2)

            out_dec_offset  = out_dec[:, :, 0:2].repeat(1, 1, 9)
            out_dec_center  = out_dec[:, :, 2:4].repeat(1,1,9)
            out_dec_36      = th.cat([out_dec_offset, out_dec_center], dim=2)
        

            epsalpha_offset = epsalpha[:, :, 0:2].repeat(1, 1, 9)
            epsalpha_center = epsalpha[:, :, 2:4].repeat(1,1,9)
            epsalpha_36  = th.cat([epsalpha_offset, epsalpha_center], dim=2)

            out_bin_start = x * xtalpha_36 - out_dec_36 * epsalpha_36 #[512, 100, 36]
            ...
            out_bin_inp = out_bin.reshape([x.shape[0], x.shape[1], (self.in_channels * self.num_bits)])
            ...
            if self.use_structural_cross_attention:
                struct_emb = struct_emb
                print(f'12456 {x_structural.shape}')
                struct_bin_input = (x_structural / 2) + 0.5 # -> [0,1]
                struct_bin_input = struct_bin_input * self.img_size #-> [0, img_size]
                struct_bin_input = dec2bin(struct_bin_input.round().int(), self.num_bits)
                print(f'NOT BASE: {x_structural.shape},{ struct_bin_input.shape}, {self.in_channels}')
                struct_bin_input = struct_bin_input.reshape([x_structural.shape[0], x_structural.shape[1], (self.in_channels//2 * self.num_bits)]) 
                struct_bin_input[struct_bin_input==0] = -1
...
@MODELS_REGISTRY.add_to_registry("TransformerPol")
class TransformerModel(nn.Module):
    """
    The full Transformer model with timestep embedding.
    """
    ...
    def expand_points(self, points: th.Tensor, connections: th.Tensor) -> th.Tensor:
        ...
        (according to the paper L=8) points from the corner along the wall to the next corner, 
        and concatenates them to the original point.
        Вход: points [N, S, 2], где 2 — [r, θ].
        Выход: [N, S, 9].
        """
        points_polar = points  #  [512, 100, 2]

        p1 = points_polar
        p1 = p1.view([p1.shape[0], p1.shape[1], 2, -1])  # [N, S, 2, 1]
        p5 = points_polar[th.arange(points_polar.shape[0])[:, None], connections[:, :, 1].long()]
        p5 = p5.view([p5.shape[0], p5.shape[1], 2, -1])  # [N, S, 2, 1]

        p3 = average_points_polar(p1, p5)  # [N, S, 2, 1]
        p2 = average_points_polar(p1, p3)
        p4 = average_points_polar(p3, p5)
        p1_5 = average_points_polar(p1, p2)
        p2_5 = average_points_polar(p2, p3)
        p3_5 = average_points_polar(p3, p4)
        p4_5 = average_points_polar(p4, p5)

        points_new_polar = th.cat((p1.view_as(points_polar), p1_5.view_as(points_polar), p2.view_as(points_polar),
                                p2_5.view_as(points_polar), p3.view_as(points_polar), p3_5.view_as(points_polar),
                                p4.view_as(points_polar), p4_5.view_as(points_polar), p5.view_as(points_polar)), dim=2)  # [N, S, 9]

        return points_new_polar.detach()
    ...
    def forward(self, x, timesteps, xtalpha, epsalpha, is_syn=False, x_structural: th.Tensor=None, **kwargs) -> typing.Tuple[th.Tensor, th.Tensor]:
        ...
        # x is  i think batch_size x num_coordinates x number of corners
        x = x.permute([0, 2, 1]).float() # -> convert [N x C x S] to [N x S x C]  - POLAR
        ...
        out_dec = self.output_linear2(out_dec)
        out_dec = self.output_linear3(out_dec)
        # out_d = polar_to_cartesian(out_dec)
